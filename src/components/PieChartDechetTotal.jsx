...
export default function PieChartDechetTotal({ className }) {
  ...
  // data & selectedCity are provided by CityContext
  useEffect(() => {
    // set a sensible default year only when none is selected yet
  ...
  useEffect(() => {
    ...
    const filtered = data.filter(d => d.city === selectedCity && Number(d.year) === Number(selectedYear))

    const sumByType = d3
      .rollups(filtered, v => d3.sum(v, d => d.value), d => d.type)
      .map(([type, value]) => ({ type, value }))
      .filter(d => d.value > 0)
      .sort((a, b) => b.value - a.value)

    drawPie(svgRef.current, wrapperRef.current, {
      items: sumByType,
      title: `Waste distribution ${selectedCity} — ${selectedYear}`
    })
  }, [data, selectedCity, selectedYear])

  return (
    <div className={`pie-card pie-dechet ${className || ''}`} ref={wrapperRef}>
      <div className="pie-header">
        <h2 className="pie-title">Waste Distribution — Total Generated</h2>
...
function drawPie(svgNode, wrapperNode, { items }) {
  ...
  // create colors array large enough for items (blend neighbors for variety)
