...
function drawPage(svgNode, wrapperNode, items){
  ...
  const bars = g.selectAll('.bp').data(items, d=>d.city).join('rect')
    ...
    .on('mouseover', function(e,d){
      ...
      tip.html(`<strong>${d.city}</strong><br/>Campaigns: ${d3.format(',')(d.campaigns)}`)
      tip.classed('show', true)
      const [mx,my] = d3.pointer(e, wrapperNode)
      tip.style('left', `${mx+14}px`).style('top', `${my+10}px`)
    })
    .on('mousemove', function(e){ const tip = d3.select(wrapperNode).select('.bar-paged-tooltip'); const [mx,my] = d3.pointer(e, wrapperNode); tip.style('left', `${mx+14}px`).style('top', `${my+10}px`) })
    .on('mouseout', function(){ const tip = d3.select(wrapperNode).select('.bar-paged-tooltip'); tip.classed('show', false); setTimeout(()=> tip.style('display','none'), 140) })
